...
- Serialized, atomic temp-file publication for immediate metadata flushes
...
6. Metadata is updated and flushed for table/write metadata changes
...
3. Client calls `commit()` to execute all operations in order
...
| Overwrite                       | Scales with resulting table size     | Uses atomic publication and may rewrite the table                                    |
...
| Index query                     | O(1) lookup after an O(n) build      | Applies only when a compatible index exists                                          |
...
- Append journals roll back partially written chunk sets
