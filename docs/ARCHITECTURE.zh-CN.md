...
**职责**：提供数据加密和解密功能，基于 `@noble/ciphers` 与本地 JavaScript 加密原语，并在独立打包环境下自动接入 `react-native-quick-crypto` 原生加速。
...
- **批量加密解密**：支持批量处理，提高效率
- **字段级加密**：支持选择性字段加密
- **Expo 环境兼容**：Expo Go 中回退到 `crypto-js` / 本地 JavaScript 实现，并使用 `expo-crypto` 生成安全随机数
