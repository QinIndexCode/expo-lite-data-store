# 更新日志

[README 入口](../README.md) | [English](./updatelog.en.md) | [消费者文档](../README.zh-CN.md)

### Unreleased - `3.0.0` 发布后加固

> 状态：以下变更发生在 npm `3.0.0` 发布之后，不属于该已发布版本。
> 存储协调：同路径文件操作和 DataWriter 表写入使用共享的进程内 FIFO，锁等待上限为 30 秒。元数据管理器另行共享按路径键控的 FIFO，重读最新快照、应用受 `createdAt` 保护的 mutation、推进跨 adapter epoch，并保留失败 mutation 供重试；超时等待者不会破坏后续队列交接。
> 恢复与删除：绑定表名的 v2 单文件 marker 记录前后代 token、hash 与物理计数；临时证据必须与持久化 metadata 和已提交主文件完全匹配。metadata backup 仅在主文件缺失时恢复，主文件损坏时绝不回退，且必须成功移除旧 backup。chunked overwrite 日志和 single-to-chunked mode 提交均有明确边界；删除后的 metadata 缺失是权威状态，同名建表会清除孤立工件。
> 完整性与安全：增量索引暂存受影响 bucket delta，重建暂存完整映射；两者都在存储前校验 `UNIQUE`，成功后才发布。动态全字段加密使用显式 metadata marker；整表逻辑计数与物理代际同次提交，解密缓存绑定精确 ciphertext。混合 legacy CTR / 当前 GCM bulk payload 会分组解密并保持顺序。加密表策略持久化，事务绑定 owner，直接提交/恢复写需要模块私有 symbol capability。
> 查询与内存边界：分页仅接受非负安全整数；所有排序算法把 nullish 值放在末尾；缓存失效使用有界命名空间版本；事务延迟 AutoSync 写入时仍保留脏条目。
> 根目录迁移安全：不可读或格式损坏的当前 `meta.ldb` 会被视为已占用；legacy 迁移前会删除空 bootstrap 根目录，避免正确性依赖 move 覆盖既有目录。
> 运行时卫生：删除未使用的存储抽象，把权限探测移出写入热路径，并让日志按级别输出、测试默认静默。

### 📅 2026-07-18 `v3.0.0` 安全边界、存储可靠性与工件卫生

> 破坏性 API 收口：移除 `plainStorage` 公开导出和包内深层导入；使用者统一从根入口调用 `db` facade 或命名 API
> 加密访问边界：加密表拒绝明文表面访问；严格 `requireAuthOnAccess` 表使用独立密钥作用域，常规与严格表之间必须由应用显式迁移并验证数据
> 存储可靠性：写入、分片迁移和删除路径采用表级互斥与更完整的回滚/缓存失效处理，读取缓存不会再把内部对象直接交给调用方修改
> 资源与测试卫生：限流器和 API payload 增加边界；QA 文本工件限制为 512 KiB 尾部、默认仅保留 3 个已完成运行，Expo consumer smoke 无论成功或失败都会清理临时目录
> 发布与文档：发布仍要求 main CI、版本精确 tag 和 `NPM_TOKEN`；中英文 API、架构、QA、CI/CD 与安全策略同步到 `3.0.0` 契约

### 📅 2026-07-15 存储可靠性与 CI/CD 恢复

> 存储可靠性：加固元数据串行可恢复发布、分片追加恢复顺序、不完整 chunk 拒绝、事务回滚语义、加密迁移和整表加密逻辑计数
> Workflow 替换：用 main push/PR 主 CI 和单独注册的 tag-only release workflow 替换被手动禁用的旧发布流程
> 发布安全：加入 tag/版本精确匹配、main 分支归属校验、显式 `NPM_TOKEN` 前置条件、完整 `prepublishOnly` 门禁、包内容检查和 npm provenance
> 维护者文档：增加中英文 CI/CD 运维手册，覆盖 Secret、发布顺序、GitHub CLI 观察和安全故障恢复
> 验证结果：58 个 Jest 套件 / 516 项测试、完整 `prepublishOnly`、Expo Doctor 21/21、Android Metro 导出、类型检查、actionlint、格式检查和 npm 包内容检查全部通过
> 跨平台 CI：修复 QA 临时目录前缀构造，显式使用 Windows 和 POSIX path 实现，并为两个模拟平台补充回归覆盖
> 干净 Checkout CI：在检查 `dist/` 的测试前先构建可分发产物，将 CI/CD 索引加入文档契约，并让剩余 simple integration 断言使用宿主路径分隔符
> Linux npm Smoke：即使 lifecycle script 先向 stdout 写入提示行，也能解析后续最终 npm JSON payload

### 📅 2026-06-28 `v2.0.2` Expo Consumer Smoke 与 GitHub 发布门禁对齐

> Expo 兼容性：将本地 React 开发依赖固定到 `19.2.3`，避免临时 Expo SDK 56 consumer app 被 npm 解析到 `expo-doctor` 拒绝的更新 patch 版本
> 发布门禁（历史记录）：`.github/workflows/npm-publish.yml` 曾在发布前执行 `npm run prepublishOnly`、用 `npm pack --dry-run --ignore-scripts` 校验 tarball，并通过 `npm publish --ignore-scripts --access public --provenance` 发布；它已在 2026-07-15 被当前 tag-only `.github/workflows/release.yml` 替换。
> GitHub 状态（历史记录）：旧 npm publish workflow 当时被手动禁用。2026-07-15 条目所述的替换已解决该状态；当前行为以 `ci.yml` 与 `release.yml` 为准。
> 验证结果：准备 2.0.2 发布元数据前，已重新执行完整构建、类型检查、全量 Jest、Expo consumer smoke、package exports smoke、workflow lint 和 npm pack 校验

### 📅 2026-06-12 `v2.0.1` Expo SDK 56 升级、存储加固与发布验证准备

> 实现审计：基于当前源码和实跑结果检查文件存储、分块存储、事务、加密、缓存、元数据与发布脚本，而不是仅依赖文档状态
> 可靠性：为分块表覆盖写入新增 overwrite journal，支持中断后的恢复；同时修复分块缓存失效、腐坏 chunk/metadata/single-file 静默吞错、事务回滚快照不完整等问题
> 安全性：将表名校验下沉到文件系统适配器边界；加密字段写入/读取改为失败即拒绝；生产环境拒绝 SecureStore 或安全随机数不可用时的非持久/不安全回退
> 性能：分块 append 不再为了计算数量读取整表，改用元数据计数路径；性能测试配置恢复覆盖真实 benchmark 文件
> 发布门禁：发布工作流改为执行完整 `prepublishOnly`，覆盖测试、构建、类型检查、Expo consumer smoke 和打包验证
> Expo 升级：将正式支持的消费端契约和本地开发工具链升级到 Expo SDK 56、React 19.2、React Native 0.85 与 TypeScript 6.0
> 验证结果：`npx expo install --check` 通过，`npm run prepublishOnly` 通过 59 个套件 / 504 个测试及压力测试、发布 smoke 检查，`npm pack --dry-run --json --ignore-scripts` 生成 `expo-lite-data-store-2.0.1.tgz`，`git diff --check` 通过
> 继续完善：将 `package-lock.json` 纳入版本控制并对齐 Expo 56 peer/dev 依赖版本下限，消除 dev audit 中的 critical；新增 `audit:prod` 与 `audit:no-high` 发布门禁
> 压力测试：`test:stress` 改为默认有界、可用 `LDS_STRESS_*` 环境变量放大、并使用可复现随机种子；该压力回归现已纳入 `test:all`

### 📅 2026-04-23 `v2.0.0` 正式版发布加固与发布验证

> 正式发布：在完成 build、typecheck、全量 Jest、Expo consumer smoke 和打包验证后，将包提升为稳定版 `2.0.0`
> 消费者契约：正式定义 Expo SDK 54 下受支持的 `npx expo install ...` 安装流程，并记录可选的 native flagship 加密依赖路径
> 打包保护：更新 `smoke:expo-consumer`，在打包前自动补齐缺失构建产物，并拒绝缺少编译产物或类型声明的 tarball
> 工具链稳定性：显式声明 `babel-preset-expo` 与 `@babel/plugin-transform-modules-commonjs`，确保本地 Jest 与发布验证行为一致
> 文档统一：统一 README、API、运行时 QA、变更日志和更新日志的消费者与维护者文档
> 验证结果：`npm run prepublishOnly`、`npm test -- --runInBand`、`npm run typecheck` 和 `npm pack --json --dry-run --ignore-scripts` 已在 Windows 环境通过

### 📅 2026-04-04 `v2.0.0-beta.5` 加密方案升级与架构优化

> 加密升级：新增 AES-256-GCM 加密模式，符合 NIST SP 800-38D 和 OWASP MASVS 2026 标准
> 自动迁移：encrypt/decrypt 自动检测数据版本，新数据默认使用 GCM，旧数据保持兼容
> 安全提升：PBKDF2 默认迭代次数从 120,000 提升到 600,000（OWASP 2026 推荐）
> 配置扩展：encryption.algorithm 支持 'AES-CTR' | 'AES-GCM' | 'auto' 三种模式
> 架构优化：解决 ConfigManager 循环依赖，创建 PathHelper 独立路径管理
> 错误处理：整合重复的 ErrorHandler 类，统一为 StorageErrorHandler 和 ApiErrorHandler
> 适配器完善：StorageAdapterFactory 支持创建 EncryptedStorageAdapter
> 配置统一：创建 tsconfig.base.json 统一所有 TypeScript 配置
> 跨平台：修复构建脚本，使用 rimraf 替代 Windows 特定命令
> 代码组织：移动 CryptoService 到 core/crypto/，删除空目录
> 新增文件：crypto-gcm.ts（GCM 加密模块）、crypto-errors.ts（错误定义）、crypto-types.ts（类型定义）
> 新增文件：PathHelper.ts（路径管理）、envUtils.ts（环境检测）
> 新增文件：.prettierignore、COMMENT_SPECIFICATION 文档（中英文）
> 测试：365 个测试全部通过，新增 GCM 加密测试

### 📅 2026-01-28 `v2.0.0-beta.3`  Expo Go 加密性能与 Provider 加固

> Expo Go 性能：降低 PBKDF2 迭代次数，加快开发迭代速度
> 原生 KDF：新增 react-native-quick-crypto，提升实际设备的 KDF 性能
> 加密 Provider：缓存原生模块加载，避免重复 require
> 加密 Provider：移除原生 PBKDF2 路径对 Buffer 的依赖，兼容 RN
> 加密 Provider：规范处理 ExpoCrypto.getRandomBytes 的不稳定返回类型
> 加密 Provider：hash 使用 TextEncoder 编码后再输入 noble
> 测试：新增 Expo Go 环境迭代次数降级行为的测试

### 📅 2026-01-22 `v2.0.0-beta.2`  加密依赖升级与架构优化

> 加密升级：从crypto-es迁移到@noble/ciphers和@noble/hashes，提升安全性和性能
> 依赖处理：简化包管理，仅使用单个package.json，移除自动生成脚本
> 文档更新：更新架构设计文档，添加加密工具模块详细描述
> 加密优化：实现AES-256-CTR + HMAC-SHA512加密方案，模拟GCM模式
> 密钥管理：优化PBKDF2密钥派生，动态调整迭代次数
> 性能提升：添加智能密钥缓存，带LRU清理策略
> 跨平台兼容：确保React Native、Expo和Web环境兼容

### 📅 2025-12-24 `v2.0.0-beta.2`  安全修复与文档更新

> 安全修复：修复了ConfigManager.ts中的原型污染漏洞，添加键名验证防止恶意键名修改Object.prototype
> 文档更新：全面更新了架构设计文档（中英文版本），添加了API层、加密层、监控层等新组件描述
> 文档更新：重绘了核心模块关系图，反映实际代码结构
> 安全策略：创建了GitHub标准的SECURITY.md文件，定义了漏洞报告流程和版本支持情况
> 测试验证：所有测试通过，确保修复不影响现有功能

### 📅 2025-12-22 `v2.0.0-beta.2`  配置系统简化与文档完善

> 文档更新：更新README.md、README.en.md、WIKI.md、WIKI_EN.md，简化配置描述和加密推荐模式
> 配置系统：移除了根目录下创建配置文件的代码，仅支持app.json配置和源代码默认配置，适配Expo Go环境
> 配置清理：移除了api.version、encryption.requireAuthOnAccess、cache.enableCompression等非必要配置
> 性能配置：启用performance.maxConcurrentOperations、performance.enableBatchOptimization、performance.memoryWarningThreshold、cache.cleanupInterval配置
> 文件清理：删除了不再需要的ConfigGenerator.ts文件
> 修复：CacheManager.ts中处理已移除的cache.enableCompression属性，返回默认值false
> 修复：ConfigManager.test.ts中移除对已删除requireAuthOnAccess属性的引用
> 修复：ConfigValidator.ts中移除对已删除配置选项的验证
> 修复：首次启动时"delete from table app_settings failed"的错误
> 包配置：调整package-env.json的exports配置，适配简化的npm上传版本
> 文档优化：简化README内容，详细内容移至WIKI，并添加加密写入/读取性能优化的Q\&A

### 📅 2025-12-20 `v2.0.0-beta.2`  性能优化与文档完善

> 修复了 crypto-security-assessment.test.ts 文件中的 TypeScript 错误
> 修复了 test:performance 命令失败的问题
> 优化了 CacheService，添加了批量标记为干净数据的方法
> 优化了 AutoSyncService，使用新的批量标记方法提高批量处理性能
> 修正了架构文档的中英文版本，添加了 CacheService 和 AutoSyncService 等新增模块
> 添加了自动同步流程的详细描述
> 确保了架构文档与实际代码的一致性
> 提高了批量处理脏数据的效率

### 📅 2025-12-18 `v2.0.0-beta.1`  主要版本更新

> 增强了加密功能，改进了字段级加密逻辑
> 移除了 enableFieldLevelEncryption 配置选项，改为自动根据 encryptedFields 决定是否启用字段级加密
> 优化了加密密钥管理和缓存机制
> 改进了文档的一致性和准确性
> 修复了构建过程中的警告和错误
> 添加了 "type": "module" 配置，支持 ES 模块
> 更新了 API 版本管理，默认版本改为 2.0.0
> 增强了生物识别认证测试覆盖
> 修复了 JEST 配置文件的 ES 模块兼容性问题
> 优化了测试覆盖率，当前覆盖率达 70.89%
> 改进了错误处理和类型定义
> 增强了安全性，确保密钥管理更加安全

<!-- 2025-12-12 修复更新、删除、删除问题 -->

### 📅 2025-12-16 `v1.1.0`  配置系统优化与文档完善

> 优化了配置系统，移除了npm install时的配置生成脚本
> 修复了配置文件在Expo项目中无法正确使用的问题
> 移除了配置API，改为直接编辑配置文件
> 优化了生物识别与密码识别触发时机，仅在实际需要时触发
> 完善了中英文文档，统一了语言使用规范
> 修正了文档中的夸大描述，确保内容准确反映项目功能
> 改进了API文档，提供了更详细的用法说明

### 📅 2025-12-12 `v1.0.5`  修复更新、删除、删除问题

> 修复了更新、删除、删除缓存问题以及其接口缺失问题。

<!-- 2025-12-12 修复配置文件生成问题 -->

### 📅 2025-12-12 `v1.0.0`  配置文件生成修复

> 修复了配置文件生成工具在不同环境下的问题，确保在所有平台上都能正常生成配置文件。
> 新增：在package.json中添加postinstall脚本，自动生成配置文件。

<!-- 1.0.1 2025-12-10 修复兼容性和参数问题 -->

### 📅 2025-12-10 `v1.0.0`  兼容与参数修复

> 修复ROOTPath.ts中获取根路径的问题以及crypto.ts中加密算法兼容性的问题。
> 修复加密兼容性问题，以及添加配置生成工具。
> 将发布依赖和开发依赖分离，实现npm publish安全发布。
> add expo version badge。

<!-- 1.0.0 2025-12-8 发布相关优化 -->

### 📅 2025-12-08 `v1.0.0`  发布优化

> 实现npm publish安全发布。
> 重构npm发布工作流。
> 更新部分文档和代码。
> 更新README，添加yarn和pnpm安装说明。
> 澄清yarn和pnpm安装说明。

<!-- 1.0.0 2025-12-7 正式发布版本 -->

### 📅 2025-12-07 `v1.0.0`  正式发布版本

> 完善README.md，提升文档质量。
> 完善部分功能描述。
> 删除测试覆盖率目录，取消测试覆盖率文件夹的提交。
> 纠正了部分API的错误实现，修复了一些性能问题。

<!-- 1.0.0 2025-12-6 核心功能完善 -->

### 📅 2025-12-06 `v1.0.0`  核心功能完善

> 修复主入口无法正确调用部分功能的问题。
> 添加Wiki文档，提升项目可维护性。
> 提升架构和系统稳定性，同时保持文档的准确性和完整性。

<!-- 1.0.0 2025-12-3 加密功能优化 -->

### 📅 2025-12-03 `v1.0.0`  加密功能优化

> 优化加密字段处理，确保在读取数据时能够正确解密，写入数据时能够正确加密。
> 更新部分信息和README.md内容。

<!-- 0.1.0 2025-11-29 测试与配置优化 -->

### 📅 2025-11-29 `v0.1.0`  测试与配置优化

> 新增：更新测试文件和配置，提升测试覆盖率。
> 优化：将chunkSize调整为5MB，提升大文件处理效率。
> 新增：从src/index.ts添加默认导出，简化API调用。
> 修正：更新README.md中的MIT协议链接和部分内容。
> 新增：添加英文README.md链接，提升国际化支持。

<!-- 0.1.0 2025-11-28 核心架构重构 -->

### 📅 2025-11-28 `v0.1.0`  核心架构重构

> 核心：重构核心架构，实现完整的存储引擎功能。
> 新增：更新文档和加密存储适配器，添加API测试。
> 清理：删除部分未使用的文件，优化项目结构。

<!-- 0.1.0 2025-11-27 代码优化 -->

### 📅 2025-11-27 `v0.1.0`  代码优化

> 优化：修改部分代码，提升系统性能和稳定性。

<!-- 0.1.0 2025-11-26 功能扩展 -->

### 📅 2025-11-26 `v0.1.0`  功能扩展

> 新增：添加缓存适配器接口、存储错误码接口。
> 新增：添加排序工具，支持数据排序功能。
> 优化：修正加密装饰器、文件系统适配器、分块文件处理器等组件。
> 新增：添加合并数据与缓存工具，提升数据处理效率。
> 重命名：将ldb.config.js重命名为liteStore.config.js。

<!-- 0.1.0 2025-11-25 基础CRUD操作 -->

### 📅 2025-11-25 `v0.1.0`  基础 CRUD

> 核心架构重构：新增文件系统适配器、分块文件处理、单文件处理、索引管理器、元数据管理器、查询引擎等核心组件。
> 完善基础CRUD操作：创建表、插入、查询、更新、删除功能稳定可用。
> 新增加密存储适配器，支持AES-CTR模式加密。

<!-- 0.0.1 2025-11-23 核心组件重构 -->

### 📅 2025-11-23 `v0.0.1`  核心组件重构

> 核心：重构核心组件，新增文件系统存储适配器、核心存储、分块文件处理、单文件处理、索引管理器、元数据管理器、查询引擎等组件。
> 优化：修改入口文件和配置文件，提升系统可用性。
> 清理：删除部分未使用的文件，优化项目结构。

<!-- 0.0.1 2025-11-19 功能扩展 -->

### 📅 2025-11-19 `v0.0.1`  功能扩展

> 新增：添加加密存储适配器，支持AES-CTR模式加密。
> 优化：简化README内容，提升文档可读性。

<!-- 0.0.1 2025-11-17 初始版本 -->

### 📅 2025-11-17 `v0.0.1`  初始版本

> 搭建基础项目骨架，实现数据交互雏形。
> 新增加密功能，支持AES-CTR模式加密，实现基础StorageAdapter接口。

<!-- 2025-11-15 项目初始化 -->

### 📅 2025-11-15  项目初始化

> 创建README.md，介绍项目基本信息。
> 进行项目初始提交，搭建基础框架。
