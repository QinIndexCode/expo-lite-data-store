...
> Transaction Boundary: The transaction owner reads its staged state through `read()`, `countTable()`, `findOne()`, and `findMany()`; filtering, sorting, pagination, and transactional `remove()` counts use that view. Queued serializable record payloads, object-based query values, and transaction query results are isolated from later caller-side mutation. On the matching active transaction surface, public `createTable()`, `deleteTable()`, and `migrateToChunked()` calls reject with `TRANSACTION_OPERATION_NOT_SUPPORTED`.
> Query and Memory Boundaries: Pagination accepts only non-negative safe integers and returns caller-visible `RangeError` validation failures; encrypted `findMany()` keeps deterministic `id` ascending order when `sortBy` is omitted; all sort algorithms keep nullish values last, cache invalidation uses bounded namespace versions, and AutoSync retains dirty entries while transactions defer writes.
> Clean-Checkout Type Contract: The tracked TypeScript configuration includes `expo/types`, so `process.env` typing does not depend on an ignored local `expo-env.d.ts` file.
...
> Publish Gate (historical): `.github/workflows/npm-publish.yml` ran `npm run prepublishOnly`, verified the tarball with `npm pack --dry-run --ignore-scripts`, and published with `npm publish --ignore-scripts --access public --provenance`; it was replaced on 2026-07-15 by the current tag-only `.github/workflows/release.yml`.
> GitHub State (historical): the prior npm publish workflow was manually disabled. The replacement recorded in the 2026-07-15 entry resolved that state; refer to `ci.yml` and `release.yml` for current behavior.
