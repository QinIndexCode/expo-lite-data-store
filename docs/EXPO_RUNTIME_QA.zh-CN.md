...
使用默认工件根目录时，harness 只会在运行完成后清理带有完成标记的旧运行，并保留最新 3 个已完成运行；进行中的运行不会被自动删除。通过 `--artifacts-dir=<path>` 指定自定义目录时不会自动清理，调用方需要自行纳入保留与清理策略。
...
- `<channel>/<profile>/<mode>/events.jsonl`
  仅包含当前 phase、当前 runId 的已接受事件；只保留尾部，最大 512 KiB
- `<channel>/<profile>/<mode>/logcat.txt`
  当前 phase 的 logcat 尾部捕获，最大 512 KiB
- `<channel>/<profile>/<mode>/screenshots/`
  启动、summary 与失败截图

发布判断应以 `summary.json` 为准，`cases.jsonl` 与各 phase 工件用于进一步定位。

`expo-doctor.log`、`expo-export.log`、`logcat.txt`、事件 JSONL 及其他文本诊断工件均使用有界尾部保留，单个文本或 JSONL 文件最多 512 KiB。较早内容可能被截断，因此需要完整原始日志时应由 CI 或设备平台在工件目录之外单独保留。
