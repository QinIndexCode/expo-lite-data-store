...
if (!global.__expo_sqlite_mock__) {
  global.__expo_sqlite_mock__ = {
    databases: {},
  };
}
...
jest.mock('expo-sqlite', () => {
  ...
  class MockSQLiteDatabase {
    ...
    async runAsync(sql, params = []) {
      const databases = global.__expo_sqlite_mock__.databases;
      const rows = databases[this.name] ?? (databases[this.name] = []);
      if (INSERT_RE.test(sql)) {
        const [tableName, id, payload] = params;
    ...
    async getFirstAsync(sql, params = []) {
      const databases = global.__expo_sqlite_mock__.databases;
      const rows = databases[this.name] ?? [];
  ...
  return {
    openDatabaseAsync: async (databaseName, _options, _directory) => {
      const databases = global.__expo_sqlite_mock__.databases;
