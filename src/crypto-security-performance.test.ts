...
describe('Expo LiteStore 加密机制安全与性能综合评估（2025）', () => {
  ...
  test('2. 批量操作加速比（100条 × 1KB）', async () => {
    console.log('\n2. 批量操作性能对比\n');

    const items = Array(100).fill(null).map(() => generateRandomString(1024));
  ...
  test('3. 字段级加密性能', async () => {
    ...
    const config = { fields: ['email', 'phone', 'password', 'idCard', 'bankCard'] as const, masterKey };

    const encTime = await measure('字段级加密（5个敏感字段）', () => encryptFields(user, { fields: ['email', 'phone', 'password', 'idCard', 'bankCard'], masterKey }));
    const encrypted = await encryptFields(user, { fields: ['email', 'phone', 'password', 'idCard', 'bankCard'], masterKey });
    const decTime = await measure('字段级解密', () => decryptFields(encrypted, { fields: ['email', 'phone', 'password', 'idCard', 'bankCard'], masterKey }));
