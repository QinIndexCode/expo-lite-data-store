...
export class ValidationWrapper {
    ...
    validateWriteData(data: Record<string, any> | Record<string, any>[]): void {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (Object.keys(item).length === 0) {
                throw new StorageError(
                    `Invalid data item at index ${i}: empty object`,
                    "FILE_CONTENT_INVALID"
                );
            }
        }
    ...
    validateFilter(filter: Record<string, any>): void {
        if (typeof filter !== "object" || filter === null) {
            throw new StorageError(
                "Invalid filter condition: filter must be an object",
                "QUERY_FAILED"
            );
        }
        ...
        if (Object.keys(filter).length === 0) {
            throw new StorageError(
                "Invalid filter condition: empty filter condition",
                "QUERY_FAILED"
            );
        }
    }
     ...
     */
    validateBulkOperations(operations: Array<{
        ...
        if (!Array.isArray(operations) || operations.length === 0) {
            throw new StorageError(
                "Invalid bulk operations: operations must be a non-empty array",
