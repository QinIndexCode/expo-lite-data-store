...
describe('Update Cache Test', () => {
  ...
  test('短时间内多次update和read操作，应始终返回最新数据', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      // 执行update
      const updatedCount = await update(TABLE_NAME, { value: newValue }, { id: 2 });
      expect(updatedCount).toBe(1);
      ...
      const item = await findOne(TABLE_NAME, { id: 2 });
  ...
  test('update操作后，不同查询条件的缓存应不受影响', async () => {
    ...
    await findOne(TABLE_NAME, { name: 'Item A' }); // 建立Item A的缓存
    await findOne(TABLE_NAME, { name: 'Item B' }); // 建立Item B的缓存

    // 3. 更新其中一条数据
    await update(TABLE_NAME, { value: 100 }, { id: 3 });

    // 4. 验证所有查询都返回最新数据
    const allData2 = await read(TABLE_NAME);
    expect(allData2.find(item => item.id === 3)?.value).toBe(100);

    const itemA2 = await findOne(TABLE_NAME, { name: 'Item A' });
    expect(itemA2?.value).toBe(100);

    // 验证其他数据不受影响
    const itemB2 = await findOne(TABLE_NAME, { name: 'Item B' });
