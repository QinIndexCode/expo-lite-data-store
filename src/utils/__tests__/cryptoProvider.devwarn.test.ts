...
describe('crypto iterations in Expo Go', () => {
  ...
  test('reduces PBKDF2 iterations for Expo Go with AES-CTR', async () => {
  ...
  test('reduces PBKDF2 iterations for Expo Go with AES-GCM', async () => {
    const pbkdf2Mock = jest.fn(
      (_password: string, _salt: Uint8Array, _iterations: number, dkLen: number, _digest: 'sha256' | 'sha512') => {
        return new Uint8Array(dkLen);
      }
    ...
    expect(pbkdf2Mock).toHaveBeenCalled();
    const iterations = pbkdf2Mock.mock.calls[0][2];
    expect(iterations).toBeLessThanOrEqual(20000);
    expect(iterations).toBeGreaterThanOrEqual(10000);
